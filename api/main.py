...
async def logout(current_user: Dict = Depends(get_current_user), authorization: Optional[str] = Header(None)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Çıkış sırasında hata oluştu")
...
async def apply_to_job(
    ...
    try:
        ...
        # Shape response to include expected 'application' object for tests
        if isinstance(result, dict) and result.get("success"):
            return {
                "success": True,
                "message": result.get("message", "Başvuru alındı"),
                "application": {
                    "id": result.get("application_id"),
                    "jobId": job_id,
        ...
        # Allow 4xx like 404/422 to propagate
        raise
    except Exception as e:
